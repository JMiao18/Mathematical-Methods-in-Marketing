...
ll <- function(beta)
{
  ...
  M1 = as.matrix(train_set)[,1:18] %*% beta[1:18] + beta[19]
  M2 = as.matrix(train_set)[,19:36] %*% beta[1:18] + beta[19]
  M3 = as.matrix(train_set)[,37:54] %*% beta[1:18] + beta[19]
...
X1_predict = as.matrix(X1_test)[,5:22] %*% ML$estimate[1:18] + ML$estimate[19]
X2_predict = as.matrix(X2_test)[,5:22] %*% ML$estimate[1:18] + ML$estimate[19]
X3_predict = as.matrix(X3_test)[,5:22] %*% ML$estimate[1:18] + ML$estimate[19]
...
ll_2 <- function(beta)
{
  ...
  M21 = as.matrix(train_set)[,1:18] %*% beta[19:36] + beta[39]
  M22 = as.matrix(train_set)[,19:36] %*% beta[19:36] + beta[39] 
  M23 = as.matrix(train_set)[,37:54] %*% beta[19:36] + beta[39]
